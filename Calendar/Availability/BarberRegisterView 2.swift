        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 16) {
                    ...
                    Button("Enregistrer le coiffeur") {
                        addBarber()
                    }
                    .padding()
                    .frame(maxWidth: .infinity)
            ...
            // ✅ REDIRECTION AUTOMATIQUE VERS LES CRÉNEAUX APRÈS ENREGISTREMENT
            .navigationDestination(isPresented: $goToAvailability) {
                BarberAvailabilityView(barberId: Auth.auth().currentUser?.uid ?? "")
    ...
    func addBarber() {
        ...
        uploadImage(image: image) { url in
            ...
            CLGeocoder().geocodeAddressString(address) { placemarks, _ in
                ...
                db.collection("barbers").addDocument(data: data) { error in
                    DispatchQueue.main.async {
                        if let error = error {
                            alertMessage = error.localizedDescription
                        } else {
                            alertMessage = "✅ Coiffeur enregistré"
